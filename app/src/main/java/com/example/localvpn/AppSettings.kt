...
object AppSettings {
    ...
    private const val KEY_SKIP_TUN_WARNING = "skip_tun_warning"
    ...
    fun getGamerTargetPackages(context: Context): Set<String> {
        ...
        val fromSet = prefs.getStringSet(KEY_GAMER_TARGET_PACKAGES, null)
            ?.map { it.trim() }
    ...
    fun setGamerTargetPackages(context: Context, packageNames: Set<String>) {
        val normalized = packageNames.map { it.trim() }.filter { it.isNotBlank() }.toSet()
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit()
    ...
    fun shouldSkipTunWarning(context: Context): Boolean {
        return context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .getBoolean(KEY_SKIP_TUN_WARNING, false)
    }

    fun setSkipTunWarning(context: Context, skip: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit()
            .putBoolean(KEY_SKIP_TUN_WARNING, skip)
            .apply()
    }
