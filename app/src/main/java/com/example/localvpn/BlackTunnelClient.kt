...
object BlackTunnelClient {
    ...
    private fun handleClient(
        ...
        logger(
            "Túnel OK name=${headers["x-name"] ?: "?"} days=${headers["x-days-left"] ?: "?"} active=${headers["x-active"] ?: "?"} dominio=$tunnelDomain"
        )
    ...
    private fun openChannel(
        ...
        ipv6ByDns.forEach { ip6 ->
            connectAndSend(
                InetSocketAddress(ip6, PROXY_PORT),
                p1,
                p2,
                protectSocket,
                "ipv6-dns:${ip6.hostAddress}",
                logger
            ).let { if (it.first != null) return it }
        }
        ...
        ipv4Direct.forEach { ip4 ->
            connectAndSend(
                InetSocketAddress(ip4, PROXY_PORT),
                null,
                p2,
                protectSocket,
                "ipv4-direct:${ip4.hostAddress}",
                logger
            ).let { if (it.first != null) return it }
        }
