...
class TunVpnService : VpnService() {
    ...
    private fun connectSafe() {
        try {
            ...
            updateForeground("Error al iniciar: ${t.message}")
            disconnect()
        }
    }
    ...
    private fun connect() {
        ...
        val localBinary = ensureBinaryForCurrentAbi() ?: run {
            ...
            updateForeground("Falta binario en assets")
            stopSelf()
            return
        }
        ...
        if (tunInterface == null) {
            ...
            updateForeground("No se pudo crear interfaz TUN")
            disconnect()
            return
        }
        ...
        Thread {
            val code = singBoxProcess?.waitFor() ?: -999
            Log.e(TAG, "sing-box terminó con código=$code")
            updateForeground("sing-box terminó: código $code")
            disconnect()
        }.start()

        updateForeground("VPN arriba; motor iniciado")
